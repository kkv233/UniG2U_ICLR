...
\usepackage{pdfpages}
% Keep the review banner outside the local group used by \maketitle.
\fancyhead[L]{Under review as a conference paper at ICLR 2027}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% THEOREMS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\theoremstyle{plain}
...
Appendix Fig.~\ref{fig:umm-taxonomy} summarizes this taxonomy and highlights how the three classes differ in their degree of unification—ranging from jointly trained parameters (E2E) to stitched modules (Decoupled), and finally to tool-invoked separate models (UM-Ag).
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.88\linewidth,height=0.40\textheight,keepaspectratio]{task.pdf}
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.86\linewidth,height=0.39\textheight,keepaspectratio]{stepwise.pdf}
...
To mechanically understand the effectiveness (or failure) of intermediate images produced during the GtA protocol, we introduce two novel alignment metrics evaluated by GPT-4o on a 1-to-5 scale, using customized rubrics for each task family. For SenseNova-U1.5-8B-MoT, the supplied RA/AL scores were produced by GPT-5.5 with the same task-specific rubrics; this evaluator difference is marked in Table~\ref{tab:rq4_alignment}.

\textbf{Reasoning-to-Visual Alignment (RA).}
RA evaluates whether the generated intermediate image correctly materializes the intended reasoning scaffold. It aggregates instruction adherence, visual quality, and task relevance (weighting schema detailed in Appendix Fig.~\ref{fig:Rubric weights}). Essentially, RA measures the \emph{fidelity of visual externalization}.

\textbf{Answer-to-Visual Alignment (AL).}
AL assesses whether the model's final answer is logically consistent with both the generated intermediate image and the original query. It aggregates visual-answer consistency, question-answer alignment, and reasoning coherence (see Appendix Fig.~\ref{fig:Rubric weights}). AL effectively measures the \emph{re-consumption capability} of the generated visual context.

\paragraph{Scoring schema.}
For both RA and AL, the evaluator produces a final score on a 5-point scale by aggregating rubric-specific sub-scores. We report the mean RA/AL scores across relevant instances, providing granular breakdowns for each model and subtask to correlate intermediate generation quality with ultimate task accuracy.

\subsection{Models and Implementation Details}
\label{sec:models-impl}

To comprehensively study the G2U effect under realistic model ecosystems, we systematically evaluate a massive suite of \textbf{36 distinct models}. These are categorized into three families: \textbf{11} base VLMs, \textbf{22} native unified models, and \textbf{3} agentic models.
...
We benchmark 22 representative open-weight unified models that natively integrate multimodal understanding and image generation within a single model interface (i.e., the model can directly follow the \textbf{Direct} and \textbf{GtA} protocols without external tool wiring). These models serve as our primary subjects for evaluating intrinsic understanding-through-generation. Crucially, as detailed in Table~\ref{tab:unified-models}, the \textbf{Backbone} column denotes the corresponding Base VLM when a paired baseline is available; SenseNova-U1.5 has no paired base result in the supplied evaluation snapshot.
...
We evaluate all 22 unified models listed in Table~\ref{tab:unified-models}, including: Emu3 \cite{wang2024emu3}, X-Omni \cite{geng2025xomni}, UniWorld-V1 \cite{lin2025uniworld}, MIO \cite{hu2025mio}, Bagel \cite{deng2025bagel}, OmniGen2 \cite{xiao2024omnigen}, Ovis-U1 \cite{wang2025ovisu1}, OneCAT-3B \cite{peng2025onecat}, JavisGPT \cite{liu2025javisgpt}, UAE \cite{yuan2025uae}, TokenFlow-XL \cite{li2024tokenflow}, UniPic2 \cite{peng2025unipic2}, Janus-Pro \cite{chen2025januspro}, Show-o2 \cite{xie2025showo2}, ILLUME+ \cite{zhang2025illume}, STAR-7B \cite{li2025star}, MMaDA \cite{yang2025mmada}, FUDOKI \cite{wang2025fudoki}, AIA \cite{zheng2025aia}, Uni-Video \cite{wei2025univideo}, MammothModa2 \cite{shen2025mammothmoda2}, and SenseNova-U1.5-8B-MoT \cite{diao2026sensenovau15}.
...
\begin{table*}[t]
\centering
\caption{\textbf{Comparison of Unified Multimodal Models.} \textcolor{cyan}{\faSnowflake}~indicates frozen backbone, \textcolor{orange}{\faUnlock}~indicates unfrozen (trainable) backbone. The \textbf{Backbone} column represents the paired Base VLM when one is available. SenseNova-U1.5 has no paired base score; OPD denotes multi-expert on-policy distillation.}
...
SenseNova-U1.5 & 2026.09 & -- & image & patch reconstruction & MoT + OPD & yes & end-to-end & N/A \\
\bottomrule
\end{tabular}%
}
\end{table*}
...
\midrule
SenseNova-U1.5-8B-MoT (Direct) & 42.50 & 44.50 & 32.50 & 22.72 & 85.00 & 29.20 & 38.82 & 36.08~(N/A) \\
SenseNova-U1.5-8B-MoT (Visual CoT) & 39.50 & 45.50 & 34.00 & 24.83 & 82.00 & 30.00 & 40.42 & 37.13~(N/A) \\
\bottomrule
\end{tabular}%
}
...
{\scriptsize \textbf{Note:} * denotes the base model. $\Delta$ is computed as overall minus the base model's overall within each block. SenseNova's $\Delta$ is N/A because no paired-base scores were supplied; Direct uses \texttt{enable\_thinking=false} and Visual CoT corresponds to the workbook's Visual CoT setting.}
...
\begin{figure*}[t]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \caption{Overall accuracy for models supporting both protocols.}
        \label{fig:rq2_overall_bar}
    \end{subfigure}\hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{Figure1_1.pdf}
        \caption{Each point is a (model, category) pair.}
        \label{fig:rq2_scatter}
    \end{subfigure}
    \caption{\textbf{GtA vs.\ Direct performance.} The left panel compares overall accuracy; the right panel shows category-level accuracy.}
\end{figure*}
...
SenseNova-U1.5-8B-MoT (GPT-5.5) & 2.06/2.98 & 2.75/3.15 & 2.36/3.00 & 2.25/2.59 & 2.36/4.26 & 2.71/3.13 & 3.30/3.64 & 2.80/3.27 \\
\bottomrule
\end{tabular}
}
\vspace{2pt}
{\scriptsize \raggedright \textbf{Evaluator note:} SenseNova-U1.5-8B-MoT RA/AL scores were produced by GPT-5.5; all other models in this table were scored by GPT-4o. SenseNova means are computed over 2,995 valid examples; five policy-refused samples are excluded.\par}
\end{table*}
...
\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.88\linewidth]{radar.pdf}
...
\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.88\linewidth]{210.pdf}
...
\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.70\linewidth]{taskbing.pdf}
...
\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.90\linewidth]{academic_apple_chart_cropped.pdf}
...
\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.84\linewidth]{case-figure.pdf}
...
\midrule
SenseNova-U1.5-8B-MoT (Direct) & 96 & 10 & 96 & 15 & 93 & 12 & 30 & 21 & 39 & 27 & 35 & 10 & 57 & 28 & 85 \\
SenseNova-U1.5-8B-MoT (Visual CoT) & 95 & 8 & 91 & 26 & 90 & 21 & 33 & 26 & 35 & 27 & 34 & 15 & 51 & 28 & 82 \\
\bottomrule
\end{tabular}%
}
\end{table*}
...
\midrule
SenseNova-U1.5-8B-MoT (Direct) & 55 & 34 & 32 & 34 & 18 & 27 & 35 & 26 & 25 & 10.84 & 0 & 62 & 0 & 27 & 38 \\
SenseNova-U1.5-8B-MoT (Visual CoT) & 65 & 26 & 30 & 37 & 18 & 23 & 42 & 29 & 28 & 12.05 & 4.33 & 62 & 0 & 36 & 32 \\
\bottomrule
\end{tabular}%
}
\end{table*}
...
\begin{table*}[!htbp]
\centering
\caption{\textbf{Delta results on UniG2U across categories (Sheet 3, Part 1/2).} Baseline models (marked with $*$) are highlighted with gray background. A dash denotes either a missing value or an unavailable delta; SenseNova-U1.5 is unpaired and therefore has no delta.}
...
gemini pro & - & - & - & - & - & - & - & - & - & - & - & - & - & - & - \\
qwenedit & - & - & - & - & - & - & - & - & - & - & - & - & - & - & - \\
SenseNova-U1.5-8B-MoT (Direct, unpaired) & - & - & - & - & - & - & - & - & - & - & - & - & - & - & - \\
SenseNova-U1.5-8B-MoT (Visual CoT, unpaired) & - & - & - & - & - & - & - & - & - & - & - & - & - & - & - \\
\bottomrule
\end{tabular}%
}
\end{table*}
...
\begin{table*}[!htbp]
\centering
\caption{\textbf{Delta results on UniG2U across categories (Sheet 3, Part 2/2).} Continuing from Part 1 with the remaining columns. A dash denotes either a missing value or an unavailable delta; SenseNova-U1.5 is unpaired and therefore has no delta.}
